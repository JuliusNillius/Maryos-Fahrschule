...
function LoginForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Vollständige Navigation statt router.push, damit keine "Load failed"-Fehler beim RSC-Fetch entstehen
        ...
        {supabaseMissing && (
          <div className="mb-6 rounded-lg border border-amber-500/40 bg-amber-500/10 p-4 text-sm text-amber-200">
            ...
            <p className="text-text-muted text-xs mb-2">
              In der Projektwurzel eine Datei <code className="rounded bg-black/30 px-1">.env.local</code> anlegen und eintragen:
            </p>
            <ul className="text-xs text-text-muted list-disc list-inside space-y-0.5">
              <li><code>NEXT_PUBLIC_SUPABASE_URL</code> (ohne Leerzeichen um =)</li>
              <li><code>NEXT_PUBLIC_SUPABASE_ANON_KEY</code> (Anon Key aus Supabase → Settings → API)</li>
            </ul>
            <p className="text-xs text-text-muted mt-2">Danach Dev-Server neu starten (npm run dev).</p>
          </div>
        )}
...
export default function BackofficeLoginPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#0A0A0A] flex items-center justify-center"><p className="text-text-muted">Lade …</p></div>}>
      <LoginForm />
    </Suspense>
  );
}
